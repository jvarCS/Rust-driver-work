        ...
        if place.layout.ty.is_unsafe_ptr() {
            match &val {
                OperandValue::Immediate(v) => {
                    eprintln!("RAW PTR LOAD: Immediate");
                    self.rawptr_metadata(*v);
                }
                OperandValue::Pair(a, b) => {
                    eprintln!("RAW PTR LOAD: Pair");
                    self.rawptr_metadata(*a);
                    self.rawptr_metadata(*b);
                }
                OperandValue::Ref(ptr, _, _) => {
                    eprintln!("RAW PTR LOAD: Ref");
                    self.rawptr_metadata(*ptr);
                }
