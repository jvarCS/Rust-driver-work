...
impl<'a, 'tcx, Bx: BuilderMethods<'a, 'tcx>> FunctionCx<'a, 'tcx, Bx> {
    ...
    fn maybe_tag_rawptr_value(
        &self,
        bx: &mut Bx,
        val: OperandValue<Bx::Value>,
    ) {
        match val {
            OperandValue::Immediate(v) => {
                eprintln!("rawptr_metadata called on Immediate");
                bx.rawptr_metadata(v);
            }
            OperandValue::Pair(v1, v2) => {
                eprintln!("rawptr_metadata called on Pair");
                bx.rawptr_metadata(v1);
                bx.rawptr_metadata(v2);
            }
            OperandValue::Ref(v, _, _) => {
                eprintln!("rawptr_metadata called on Ref");
                bx.rawptr_metadata(v);
            }
            OperandValue::ZeroSized => {
                eprintln!("rawptr_metadata called on zerosized");
            }
        }
    }
    // RAW PTR STUFF
